...
function preload() {
    ...
    cropSprites = {
        ...
        'beetroot': [
            loadImage('js/images/crops/beetroot_chopped/beetroot_00.png'),
            loadImage('js/images/crops/beetroot_chopped/beetroot_01.png'),
            loadImage('js/images/crops/beetroot_chopped/beetroot_02.png'),
            loadImage('js/images/crops/beetroot_chopped/beetroot_03.png'),
            loadImage('js/images/crops/beetroot_chopped/beetroot_04.png'),
            loadImage('js/images/crops/beetroot_chopped/beetroot_05.png'),
        ],
        'cauliflower': [
            loadImage('js/images/crops/cauliflower_chopped/cauliflower_00.png'),
            loadImage('js/images/crops/cauliflower_chopped/cauliflower_01.png'),
            loadImage('js/images/crops/cauliflower_chopped/cauliflower_02.png'),
            loadImage('js/images/crops/cauliflower_chopped/cauliflower_03.png'),
            loadImage('js/images/crops/cauliflower_chopped/cauliflower_04.png'),
            loadImage('js/images/crops/cauliflower_chopped/cauliflower_05.png'),
        ],
        'carrot': [
    ...
    shopItemSprites = {
        'beetroot_seed': loadImage('js/images/items/beetroot_seed.png'),
        'cauliflower_seed': loadImage('js/images/items/cauliflower_seed.png'),
        'corn_seed': loadImage('js/images/items/corn_seed.png'),
        'crops_seed': loadImage('js/images/items/crops_seed.png'),
        'energy_potion': loadImage('js/images/items/energy_potion.png'),
        'wheat_seed': loadImage('js/images/items/wheat_seed.png')
    };

    npcSprites = {
        'blacksmith': loadImage('js/images/npcs/blacksmith.png')
    };

    for (let npcId in NPCConfigs) {
        if (npcSprites[npcId]) {
            NPCConfigs[npcId].sprite = npcSprites[npcId];
        }
    }
}
...
function areAllResourcesLoaded() {
    ...
    for (let itemKey in shopItemSprites) {
        if (!shopItemSprites[itemKey] || !shopItemSprites[itemKey].width) {
            return false;
        }
    }

    for (let npcId in npcSprites) {
        if (!npcSprites[npcId] || !npcSprites[npcId].width) {
            return false;
        }
    }
...
function draw() {
    ...
    npcManager.checkInteractions(player);
...
function keyPressed() {
    if (key === '1') {
        ...
        player.action();
    } else if (key === 'G' && keyIsDown(SHIFT) && debugMode) {
        const mouseWorldX = mouseX + cameraX;
        const mouseWorldY = mouseY + cameraY;
        
        for (let field of fields) {
            if (field.containsPoint(mouseWorldX, mouseWorldY) && field.crop) {
                field.crop.growthStage = field.crop.maxGrowthStage;
                field.crop.isReadyToHarvest = true;
                player.tipBox.show("Crop has grown to maturity!");
                break;
            }
        }
    } else if (key === 'N' && debugMode) {
        // Skip to next morning
        gameManager.hour = 23;
        gameManager.minute = 59;
        ...
        player.tipBox.show("Time skipped to 8am next day!");
        console.log('Debug: Skipped to next day morning');
    } else if (key === 'd' || key === 'D') {
        debugMode = !debugMode;
    } else if (key === 'v' || key === 'V') {
        ...
        const crops = ['corn', 'wheat', 'beetroot', 'cauliflower'];
...
function mouseClicked() {
    ...
    shopUI.handleClick(mouseX, mouseY);
    taskUI.handleClick(mouseX, mouseY);
    achievementUI.handleClick(mouseX, mouseY);
    uiToolbar.handleClick(mouseX, mouseY);
}
